...
public class ParallelxBackground extends Component {
    ...
    public ParallelxBackground(String file, GameObject[] backgrounds, Ground ground, boolean followGround) {
        ...
        if (this.gameObject.transform.position.x < -width) {
            ...
            for (GameObject go : backgrounds) {
                if (go.transform.position.x > maxX) {
                    ...
                    otherTimeStep = go.getComponent(ParallelxBackground.class).timeStep;
                }
            }
